...
class Config:
    ...
    training_NN = {"ETHUSD"}  # тикеры по которым обучаем нейросеть
    portfolio = {"ETHUSD"}  # тикеры по которым торгуем и скачиваем исторические данные
    ...
    timeframe_1 = "H1"  # таймфрейм для обучения нейросети - выход
    start = "2021-01-01"  # с какой даты загружаем исторические данные с MOEX

    trading_hours_start = "00:00"  # время работы биржи - начало
    trading_hours_end = "23:59"  # время работы биржи - конец
